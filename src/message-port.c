		...
		message_port_remote_app_info_s **mri, port_list_info_s **pli)
{
	...
	*mri = remote_app_info;
	...
	if (cb_list == NULL) {
		...
		_LOGE("@@@@@@@@@@@@@@@@@@@@@@@@@@@ port_list len %d", g_list_length(remote_app_info->port_list));
...
static bool send_message(GVariant *parameters, GDBusMethodInvocation *invocation)
{
	...
	_LOGE("@@@@@@@@@@@@@@@@@@@@@@@@@@@ local_port %d, remote_app %d, sender %d, trusted %d ", g_hash_table_size(__local_port_info)
		, g_hash_table_size(__remote_app_info)
		, g_hash_table_size(__sender_appid_hash)
		, g_hash_table_size(__trusted_app_list_hash));
